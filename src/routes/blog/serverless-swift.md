...
I built a serverless back-end system entirely in Swift that remotely controls the Philips Hue bulbs in my house whenever my favorite sports teams score or win. It runs on AWS services including Lambda, DynamoDB, SQS, and EventBridge.
...
So many reasons that I could talk about for days. I love building serverless systems. Once you understand the primitives, serverless offers the best combination of speed, power, fun, and scale that I know of. It's not a cure-all, but it's great for projects like this.
...
As mentioned above, it's a serverless back-end system written in Swift that remotely controls the Philips Hue bulbs in my house whenever my favorite sports teams score or win. It runs on AWS services like Lambda, DynamoDB, SQS, and EventBridge.

I used Xcode to build it, but any editor that supports the [SourceKit-LSP](https://github.com/swiftlang/sourcekit-lsp) will work - including VS Code, Neovim, Emacs, Nova, and others.
...
This is where you configure the resource primitives you want to use, like functions, databases, queues, and more.

Swift Cloud provides these components for you. AWS is my cloud of choice, though they also offer providers for Cloudflare, Digital Ocean, Fastly, and Vercel.
...
We can use Swift's pattern matching with `guard case` to safely extract values from the DynamoDB records (`newImage` and `oldImage`) like this:
...
Hue's API only allows for controlling one light at a time, and this is where **Swift Concurrency** comes in. Using [TaskGroup](https://developer.apple.com/documentation/swift/taskgroup) or [async let](https://www.hackingwithswift.com/quick-start/concurrency/how-to-call-an-async-function-using-async-let) syntax, we can easily and safely fire off multiple API requests in parallel (one for each light we need to control) and wait for them all to complete before returning from our Lambda.
...
All of that costs **less than 20 cents per month** to run on AWS. Essentially all of the cost is from DynamoDB, which I could be smarter about to save money if I cared about 20 cents per month.

Further, because this is fully serverless, it will scale to zero for a large chunk of the year. The first step of my poller function is to check whether it's currently football or basketball season, and, if it's not, return immediately and don't trigger the rest of the flow. At that point, the system will automatically scale down to $0/month without me doing anything whatsoever.
...
Each log stream contains one cold start (its first invocation), and then each subsequent invocation inside the stream is from an already-warm container - so there are no cold starts after the first one in every log stream. Therefore, we'll look at the duration and cold starts of just the first invocation of each log stream - which theoretically should have the **worst** performance of any of the invocations in each stream.
...
The full repository, including everything discussed in this post and much more, is on [GitHub](https://github.com/r-token/sports-home-automation-swift).
